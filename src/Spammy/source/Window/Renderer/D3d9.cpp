#include "Spammy/Window/Renderer/D3d9.h"

WindowRenderer_D3d9::WindowRenderer_D3d9() : _hwnd(NULL), _lib(NULL), _dev(NULL)
{
    memset(&_params, NULL, sizeof(_params));
    _params.Windowed = TRUE;
    _params.SwapEffect = D3DSWAPEFFECT_DISCARD;
    _params.BackBufferFormat =
...
bool WindowRenderer_D3d9::create(HWND hwnd)
{
    ...
    HRESULT hRes = _lib->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hwnd, D3DCREATE_HARDWARE_VERTEXPROCESSING,
                                      &_params, &_dev);
    if (hRes < 0) {
        _lib->Release();
        _lib = NULL;
...
void WindowRenderer_D3d9::render()
{
    ...
    if (result == D3DERR_DEVICELOST && _dev->TestCooperativeLevel() == D3DERR_DEVICENOTRESET) reset();
}
